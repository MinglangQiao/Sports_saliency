...
class SwinTransformer3D(nn.Module):
    ...
    def forward(self, x, return_fea=False):
        """Forward function."""
        layer_features = []
        middle_features = {} if return_fea else None

        x = self.patch_embed(x)
        if return_fea:
            middle_features['patch_embed'] = x.clone()

        x = self.pos_drop(x)
        if return_fea:
            middle_features['pos_drop'] = x.clone()

        count_layer = 0 if return_fea else None
        for layer in self.layers:
            x, x1 = layer(x.contiguous())
            layer_features.append(x1)

            if return_fea:
                middle_features['layer{}'.format(count_layer)] = x1.clone()
                count_layer += 1

        x = rearrange(x, 'n c d h w -> n d h w c')
        x = self.norm(x)
        if return_fea:
            middle_features['norm'] = x.clone() 

        x = rearrange(x, 'n d h w c -> n c d h w')

        if return_fea:
            return x, layer_features, middle_features
        else:
